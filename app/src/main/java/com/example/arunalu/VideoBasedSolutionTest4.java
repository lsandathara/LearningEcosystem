...
public class VideoBasedSolutionTest4 extends AppCompatActivity {
    ...
    String actualSentence = "මමයි පුංචි කේතලේ";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_video_based_solution_test4);

        playGuidanceAudio();
        VideoView kettleVideo = (VideoView)findViewById(R.id.kettle_video);

        kettleVideo.setVideoURI(Uri.parse("android.resource://"+getPackageName()+"/"+R.raw.kettle_video));

        kettleVideo.start();

        kettleVideo.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
            @Override
        ...
        speechRecognizer = SpeechRecognizer.createSpeechRecognizer(VideoBasedSolutionTest4.this);
    ...
    private void getVoiceInput(){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            if (!(ContextCompat.checkSelfPermission(VideoBasedSolutionTest4.this, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED)){
...
//        bundle.putString("time", timeText);
        className = "VideoBasedSolutionTest4";
